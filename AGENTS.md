...
- **Docker image**: Set `MOCK_LLM_DOCKER_IMAGE` to the image tag (default: `ghcr.io/asahinayui/cross-agent-control-plane:latest`). The container is started with `--rm --network host` and a unique `--name` for cleanup.
