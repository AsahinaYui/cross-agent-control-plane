...
const DEFAULT_CONTROL_PLANE_PORT = SHARED_DEFAULTS.ports.controlPlane;
...
async function buildConfig(args, env = process.env) {
  ...
  const launchControlPlane = !frontendOnly;
  ...
  const preferredControlPlanePort =
    parseInt(env.CROSS_AGENT_CONTROL_PLANE_PORT, 10) ||
  ...
  if (launchControlPlane) {
    requiredPorts.push({
      name: "control-plane",
      port: preferredControlPlanePort,
    });
  }
  if (launchFrontend) {
  ...
  return {
    ...
    controlPlanePort: preferredControlPlanePort,
...
const CONTROL_PLANE_ROUTE_PREFIX = "/api/control-plane";
...
function getLocalServiceRoutes(config) {
  ...
  if (config.launchControlPlane) {
    routes.push([
      CONTROL_PLANE_ROUTE_PREFIX,
      `http://127.0.0.1:${config.controlPlanePort}`,
    ]);
  }
...
function getRejectPrefixes(config) {
  ...
  if (!config.launchControlPlane) {
    prefixes.push(CONTROL_PLANE_ROUTE_PREFIX);
  }
  if (!config.launchAgentServer) {
    for (const prefix of AGENT_SERVER_ROUTE_PREFIXES) {
...
function startControlPlane(config) {
  logService(
    "control-plane",
    `Starting on port ${config.controlPlanePort}...`,
    c.cyan,
  );
  spawnService(
    "control-plane",
    "node",
    [
      join(projectRoot, "control-plane", "cli.mjs"),
      "serve",
      "--state",
      join(config.stateDir, "control-plane"),
      "--port",
      String(config.controlPlanePort),
    ],
    {
      cwd: projectRoot,
      env: { CONTROL_PLANE_API_KEY: config.sessionApiKey },
      color: c.cyan,
    },
  );
}
...
async function main(options = {}) {
  ...
  // Control Plane is an independent local service. It does not share
  // lifecycle state or persistence with Agent Server.
  if (config.launchControlPlane) {
